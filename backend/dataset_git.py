...
class dataset_git:
    ...
    def get_root(self)->str:
        #self.version_ids = [version["id"] for version in Dataset.list_datasets([self.pull_dataset_project], only_completed=True)]            
        root = [item for item, value in self.latest_dataset.get_dependency_graph().items() if value==[]]
        return root[0]
    
    def create_empty_dataset_task(self, dataset_name, parents=[]):
        dataset = Dataset.create(dataset_name=dataset_name, dataset_project=self.pull_dataset_project, parent_datasets=parents)
        return dataset

    def get_dataset(self, dataset_name:str, auto_create=False, writable_copy=False, only_completed=False)->Dataset:
        try:
            dataset_obj = Dataset.get(dataset_project=self.pull_dataset_project, dataset_name=dataset_name, auto_create=auto_create, writable_copy=writable_copy, only_completed=only_completed)
            return dataset_obj
    ...
    def delete_dataset(self, dataset_name:str=None, dataset_id:str=None)->None:
        try:
            Dataset.delete(dataset_project=self.pull_dataset_project, dataset_name=dataset_name, dataset_id=dataset_id)
        except Exception as e:
            return e
    ...
    def get_latest_dataset(self, target_folder_path:str)->Dataset:
        ...
        return self.latest_dataset.get_mutable_local_copy(target_folder_path, overwrite=True)
...
class annotator_controller(dataset_git):
    ...
    def init_source_raw(self, project_name:str) -> None:
        ...
        self.source_raw_id = self.create_root(dataset_name="source_raw", dataset_project=self.pull_dataset_project, upload_path="./source_raw")

    def init_target_raw(self, project_name:str) -> None:
        deletion_error = self.delete_dataset(dataset_name="target_raw")
        self.target_raw_id = self.create_root(dataset_name="target_raw", dataset_project=self.pull_dataset_project, upload_path="./target_raw")
    ...
    def set_source_mentions(self, source_mentions)->None:
        ...
        if source_mentions_dataset==None:
            source_mentions_dataset=self.create_empty_dataset_task("source_mentions", parents=[self.get_source_raw().id])
            source_mentions_dataset.finalize()
        source_mentions_dataset.get_mutable_local_copy(self.mentions_source_path, overwrite=True)
        json.dump(source_mentions, open("{}/source_mentions.json".format(self.mentions_source_path), "w"))

        new_source_mentions_dataset = self.create_empty_dataset_task("new_source_mentions", parents=[source_mentions_dataset.id])
        ...
        self.squash_datasets("source_mentions", [source_mentions_dataset.id, new_source_mentions_dataset.id])
    ...
    def set_target_mentions(self, target_mentions)->None:
        ...
        if target_mentions_dataset==None:
            target_mentions_dataset=self.create_empty_dataset_task("target_mentions", parents=[self.get_target_raw().id])
            target_mentions_dataset.finalize()                    
        target_mentions_dataset.get_mutable_local_copy(self.mentions_target_path, overwrite=True)
        json.dump(target_mentions, open("{}/target_mentions.json".format(self.mentions_target_path), "w"))

        new_target_mentions_dataset = self.create_empty_dataset_task("new_target_mentions", parents=[target_mentions_dataset.id])
        ...
        self.squash_datasets("target_mentions", [target_mentions_dataset.id, new_target_mentions_dataset.id])
